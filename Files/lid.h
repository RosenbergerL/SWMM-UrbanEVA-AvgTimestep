...
//
//  Build UrbanEva (ue):
//   - Structure TVEGETATLayer added
//   - TVEGETATLayer VEGETAT added to TLidProc
//   - Parameters for ue added to TWaterBalance and TLidUnit
//   - Parameter rain added to lidproc_getOutflow for interception calculation
//
//-----------------------------------------------------------------------------
...
// Vegetation Layer for UrbanEva surface Layser			                          //(ue)
typedef struct
{
    double    laimean;            // annual mean of LAI
    double    cropfactor;         // cropfactor for etp reduction
    double    SL;				  // rainfall interception / leaf storage coefficient
    double    nfcThreshold;       // threshold soil moisture for maximum evaporation
    int       vegetationpattern;  // index of pattern for annual vegetation     //(ue 2024)
    int       evaporationType;    // method for evaporation 0:swmm default 1:linear 2:exponential 3: potential 4: combined exp Disse 5: combined linear Ostrowski
    int       interceptionType;   // method for interception 0: without interception 1: Rainfall 2: SCF*Rainfall 3: I-Rate by Braden
    int       intercepEvapType;   // method for evap interceptionn 1: ETp 2: f(si)*ETp ; is ignored if interception=0
    int       transpirationType;  // method for transpiration evaporation 1:1-SCF 2: f(soil moisutre); is ignored if evaporation= 0|4|5
    int       evapWhileponding;   // calc evaporation for interception and transpiration while ponding 1: TRUE 0: FALSE
    int       infiltrationType;   // method for infiltration 1: GreenAmp 2: Holtan
}  TVEGETATLayer;
...
typedef struct
{
    ...
    TVEGETATLayer  VEGETAT;       // Vegetation Layer                          //(ue)
}  TLidProc;
...
typedef struct
{
    ...
    double         intercepEvap;  // total interception Evap (ft)              //(ue)   
    double         soilEvap;      // total soil evap (ft)                      //(ue)
    double         soilTransp;    // total transpiration (ft)                  //(ue)
    double         surfaceEvap;   // total surface evap (ft)                   //(ue)
}  TWaterBalance;
...
typedef struct
{
    ...
    double   intercepDepth;  // depth of water in interception storage layer (ft)   //(ue)
    double   intercepInflow; // inflow tointerception storage layer (ft/s)          //(ue)
    double   intercepEvap;   // evaporation from interception storage layer (ft/s)  //(ue)
    double   intercepMaxDepth; // current Smax value for montly pattern of LAI (ft) //(ue)
    double   intercepSCF;    // soil cover fraction by vegetation (-)               //(ue)
    double   intercepLAI;    // LAI of vegetation for actual pattern                //(ue)
