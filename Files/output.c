...
// 
//   Build UrbanEva-AvgTimestep (ue-avg):
//   - New functions output_initTxtFile and output_closeTxtFile for text file export
//   - New function output_saveAvgSubcatchResults for saving average subcatchment results 
// 
//-----------------------------------------------------------------------------
...
extern char      InputFileName[512];   // input file name from swmm5.c              //(ue-avg)
...
static TAvgResults* AvgSubcatchResults;     //(ue-avg)
static int          Nsteps;
...
static void output_saveAvgSubcatchResults(double reportTime, FILE* file);     //(ue-avg)
static void output_closeTxtFile();                                            //(ue-avg)
static void output_initTxtFile();                                             //(ue-avg)
...
//
{
    ...
    if (RptFlags.txtfile) output_initTxtFile();                             //(ue-avg)     
    ...
    AvgNodeResults = NULL;                                                  //(5.1.013)
    AvgLinkResults = NULL;                                                  //
    AvgSubcatchResults = NULL;                                              //(ue-avg)
    if (RptFlags.averages && !output_openAvgResults())                      //(5.1.013)
    {                                                                       //
        report_writeErrorMsg(ERR_MEMORY, "");                               //
        return ErrorCode;                                                   //
    }
...
//
{
    ...
    if (RptFlags.txtfile) output_closeTxtFile();                                                //(ue-avg)
}

//=============================================================================

void output_initTxtFile() {                                                                     //(ue-avg)
    char txtFileName[512];                                                                      //
    char* dotPos;                                                                               //

    strncpy(txtFileName, InputFileName, sizeof(txtFileName) - 1);                               //
    txtFileName[sizeof(txtFileName) - 1] = '\0';                                                //

    dotPos = strrchr(txtFileName, '.');                                                         //
    if (dotPos != NULL && strcmp(dotPos, ".inp") == 0)                                          //
    {                                                                                           //
        *dotPos = '\0';                                                                         //
    }                                                                                           //
    strncat(txtFileName, "_Results_Subc.txt", sizeof(txtFileName) - strlen(txtFileName) - 1);   //

    // Open the text file for writing   
    txtFile.file = fopen(txtFileName, "w");                                                     //            
    if (txtFile.file == NULL) {                                                                 //
        printf("Error: Unable to open output.txt for writing.\n");                              //
        exit(1);                                                                                //
    }                                                                                           //

    fprintf(txtFile.file, "SWMM5 Simulation Output Report\n\n");                                //
    fprintf(txtFile.file, "********************************\n");                                //
    fprintf(txtFile.file, "Subcatchment Timeseries Results\n");                                 //
    fprintf(txtFile.file, "********************************\n");                                //

    static int colCount = 11;                                                                   //
    static char* head1[] = {                                                                    //
    "\n          \t", "        \t", "            \t",                                           //
    "Rainfall\t", "SnowDepth\t",                                                                //
    "Evaporation\t", "Infiltration\t", "Runoff\t",                                              //
    "GroundwFlow\t", "GroundwElev\t", "SoilMoisture\t"                                          //
    };                                                                                          //

    static char* headCMS[] = {                                                                  //
    "\nDate      \t", "Time    \t", "Subcatchment\t",                                           //
    "mm/hr   \t",    "mm         \t",                                                           //
    "mm/d       \t", "mm/hr       \t", "m³/s  \t",                                              //
    "m³/s       \t", "m          \t",  "Percent     \t"                                         //
    };                                                                                          //
    ...
    if (FlowUnits == CMS || FlowUnits == LPS || FlowUnits == MLD) {                             //
        selectedHeader = headCMS; // Metric units                                               //
    }                                                                                           //
    else {                                                                                      //
        selectedHeader = headUS; // US customary units                                          //
    }                                                                                           //

    for (int i = 0; i < colCount; i++) {                                                        //
        fprintf(txtFile.file, "%s", head1[i]);                                                  //
    }                                                                                           //

    for (int i = 0; i < colCount; i++) {                                                        //
        fprintf(txtFile.file, "%s", selectedHeader[i]);                                         //
    }                                                                                           //

    fprintf(txtFile.file, "\n-------------------");                                             //
    for (int i = 0; i < colCount; i++) fprintf(txtFile.file, "------------");                   //
    fprintf(txtFile.file, "\n");                                                                //
...
void output_closeTxtFile() {                                                //(ue-avg)
    if (txtFile.file != NULL && txtFile.state == 'O') {                     //
        fclose(txtFile.file);                                               //
        txtFile.file = NULL;                                                //
        txtFile.state = 'C';                                                //
    }                                                                       //
    else {                                                                  //
        printf("Warning: txtFile was not open or is already closed.\n");    //
    }                                                                       //
}                                                                           //
...
//
{
    ...
    if (RptFlags.averages) output_saveAvgSubcatchResults(reportTime, Fout.file);      //(ue-avg)

    if (RptFlags.txtfile && !RptFlags.averages)                                       //(ue-avg)
    {
        for (j = 0; j < Nobjects[SUBCATCH]; j++)
        {
            if (Subcatch[j].rptFlag)
            {
                ...
                {
                    DateTime reportDate;
                    if (ReportTime - 1000 * ReportStep < StartDateTime)
                        reportDate = StartDateTime;
                    else
                        reportDate = getDateTime(ReportTime - 1000 * ReportStep);

                    int year, month, day, hour, minute, second;
                    datetime_decodeDate(reportDate, &year, &month, &day);
                    datetime_decodeTime(reportDate, &hour, &minute, &second);
                                        
                    fprintf(txtFile.file, "%04d-%02d-%02d\t%02d:%02d:%02d\t%-15s\t",
                        year, month, day, hour, minute, second, Subcatch[j].ID);

                    subcatch_getResults(j, ReportTime, SubcatchResults);
                    for (k = 0; k < NumSubcatchVars; k++)
                    {
                        fprintf(txtFile.file, "%.5f\t", SubcatchResults[k]);
                    }
                    fprintf(txtFile.file, "\n");
                }
                else
                {
                    printf("Error: txtFile is not open for writing.\n");
                }
            }
        }
    }
    ...
    for (j = 0; j < Nobjects[SUBCATCH]; j++)
    {
        ...
        if (!RptFlags.averages)                                                                     //(ue-avg)
        {
            if (Subcatch[j].rptFlag) fwrite(SubcatchResults, sizeof(REAL4), NumSubcatchVars, file); //(ue-avg)
        }
...
int output_openAvgResults()
{
    ...
    // --- allocate memory for averages at reportable subcatchments
    AvgSubcatchResults = (TAvgResults*)calloc(NumSubcatch, sizeof(TAvgResults));        //(ue-avg)
    if (AvgSubcatchResults == NULL) return FALSE;                                       //
    for (i = 0; i < NumSubcatch; i++) AvgSubcatchResults[i].xAvg = NULL;                //
    ...
    // --- allocate memory for each reportable variable for each reportable subcatchment
    for (i = 0; i < NumSubcatch; i++)                                                   //(ue-avg)
    {                                                                                   //
        AvgSubcatchResults[i].xAvg = (REAL4*)calloc(NumSubcatchVars, sizeof(REAL4));    //
        if (AvgSubcatchResults[i].xAvg == NULL)                                         //
        {                                                                               //
            output_closeAvgResults();                                                   //
            return FALSE;                                                               //
        }                                                                               //
...
{
    int i;
    if (AvgSubcatchResults)                                                            //(ue-avg)
    {                                                                                  //
        for (i = 0; i < NumSubcatch; i++) FREE(AvgSubcatchResults[i].xAvg);            //
        FREE(AvgSubcatchResults);                                                      //
    }                                                                                  //
    if (AvgNodeResults)
    {
...
void output_initAvgResults()
{
    ...
    for (i = 0; i < NumSubcatch; i++)                                               //(ue-avg)
    {                                                                               //
        for (j = 0; j < NumSubcatchVars; j++) AvgSubcatchResults[i].xAvg[j] = 0.0;  //
    }                                                                               //
    for (i = 0; i < NumNodes; i++)
    {
...
void output_updateAvgResults()
{
    ...
    // --- update average accumulations for subcatchments
    k = 0;                                                                          //(ue-avg)
    if (NewRoutingTime = NewRunoffTime)                                             //
    {                                                                               //
        for (i = 0; i < Nobjects[SUBCATCH]; i++)                                    //
        {                                                                           //
            if (!Subcatch[i].rptFlag) continue;                                     //
            subcatch_getResults(i, 1.0, SubcatchResults);                           //
            for (j = 0; j < NumSubcatchVars; j++)                                   //
            {                                                                       //
                AvgSubcatchResults[k].xAvg[j] += SubcatchResults[j];                //
            }                                                                       //
            k++;                                                                    //
        }                                                                           //
    }                                                                               //
...
void output_saveAvgSubcatchResults(double ReportTime, FILE* file)                       //(ue-avg)
{                                                                                       //
    int i, j, k;                                                                        //
    int rptCounter = 0;                                                                 //
        ...
        {                                                                               //
            ...
            {                                                                           //
                ...
                {                                                                       //
                    ...
                    for (k = 0; k < NumSubcatchVars; k++)                               //            
                    {                                                                   //
                        REAL4 value = AvgSubcatchResults[rptCounter].xAvg[k] / Nsteps;  //
                        fprintf(txtFile.file, "%.5f\t", value);                         //            
                    }                                                                   //    
                    fprintf(txtFile.file, "\n");                                        //    
                }                                                                       //
                else                                                                    //
    ...
    k = 0;                                                                              //
    for (i = 0; i < NumSubcatch; i++)                                                   //    
    {                                                                                   //
        for (j = 0; j < NumSubcatchVars; j++)                                           //
        {                                                                               //    
            SubcatchResults[j] = AvgSubcatchResults[k].xAvg[j] / Nsteps;                //
        }                                                                               //
