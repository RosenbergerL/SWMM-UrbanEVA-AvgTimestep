...
//
//  Build UrbanEva (ue):
//   - Enum LidLayerTypes and LidRptVars
//   - Adding local variables
//   - Adding functions(i) getEvapRatesIntercep and (ii)getInterceptionRate
//   - lidproc_initWaterBalance: init UE variables
//   - lidproc_getOutflow : add parameter rain, basic calculation of UE parameters, 
//     switch off of infiltration calculation for biocell, 
//     store results, modification of balancingand flux - rates
//   - lidproc_saveResults : implement interception
//   - biocellFluxRates : adding UE variables
//-----------------------------------------------------------------------------
...
enum LidRptVars {
    ...
    VEGE_DEPTH,              // water level in interception layer              //(ue)  
    VEGE_INFLOW,             // inflow to interception storage     	           //(ue)  
    VEGE_EVAP,               // evaporation from interception layer	           //(ue)
    SOIL_EVAP,               // evaporation from soil layer	                   //(ue)
    SOIL_TRANSEVAP,          // transpiration via root water uptake            //(ue)
    SURF_EVAP,               // evaporation from surface layer                 //(ue)
    EP,                      // potential values Ep                            //(ue)
    EIP,                     // potential values Eip                           //(ue)
    ESP,                     // potential values EsP                           //(ue) 
    ETP,                     // potential values Etp                           //(ue) 
...
static double     Rainfall;       // current rainfall rate (ft/s)              //(ue)
static double     EvapRate;       // evaporation rate (ft/s)
static double     MaxNativeInfil; // native soil infil. rate limit (ft/s)

static double     IntercepRate;   // outflow form interception storage (ft/s)  //(ue)
static double     IntercepEvap;   // evap. rate from interception storage (ft/s)  //(ue)
static double     IntercepVolume; // volume in interception storage (ft)       //(ue)
...
static double     SoilTransEvap;  // transp. from soil layer by root water uptake (ft/s) //(ue)
...
static double     potEp;          // pot. evaporations rate potEp  (ft/s)      //(ue)
static double     potEip;         // pot. evaporations rate potEip (ft/s)      //(ue)
static double     potEsp;         // pot. evaporations rate potEsp (ft/s)      //(ue)
static double     potEtp;         // pot. evaporations rate potEtp (ft/s)      //(ue)
...
static void   getEvapRatesIntercep(double surfaceVol, double intercepVol,      //(ue)
    double soilVol, double storageVol, double pervFrac);
static double getInterceptionRate(double depth);                               //(ue)
...
//
{
    ...
    lidUnit->waterBalance.intercepEvap = 0.0;                                  //(ue) 
    lidUnit->waterBalance.soilEvap = 0.0;	                                   //(ue)
    lidUnit->waterBalance.soilTransp = 0.0;	                                   //(ue)
    lidUnit->waterBalance.surfaceEvap = 0.0;                                   //(ue)
...
double lidproc_getOutflow(TLidUnit* lidUnit, TLidProc* lidProc,
    double rain, double inflow,                          //(ue) add parameter rain
    ...
    //
{
    ...
    double xTol[MAX_LAYERS] = { STOPTOL, STOPTOL, STOPTOL, STOPTOL, STOPTOL };   //(ue)
    ...
    x[VEGE] = theLidUnit->intercepDepth;                                       //(ue)
    ...
    // ... if lidType is Bio-Cell than use UrbanEva modification               //(ue)
    if (theLidProc->lidType == BIO_CELL)
    {
        // ...calc Smax as max interception, store in structure theLidUnit
        // ... Vegetation Layer properties, added for interception calculation         
        int month = datetime_monthOfYear(getDateTime(NewRunoffTime)) - 1;
        double facmonth = Pattern[theLidProc->VEGETAT.vegetationpattern].factor[month];
        theLidUnit->intercepLAI = theLidProc->VEGETAT.laimean * facmonth;

        //... current soil cover fraction
        theLidUnit->intercepSCF = (1.0 - pow(0.7, theLidUnit->intercepLAI));
        //... current max interception and convert to ft
        theLidUnit->intercepMaxDepth = theLidProc->VEGETAT.SL * theLidUnit->intercepLAI / UCF(RAINDEPTH);

        xMax[VEGE] = theLidUnit->intercepMaxDepth;
        //... substract Rainfall, Rainfall is on vegetation
        SurfaceInflow -= Rainfall;
    ...
    else if (theLidUnit->soilInfil.Ks > 0.0 && theLidProc->lidType != BIO_CELL)    //(ue)
    {
        SurfaceInfil =
    ...
    *******************************************************************/
    ...
    theLidUnit->intercepDepth = x[VEGE];                                       //(ue)

    //... save some more variables for interception development                //(ue)
    theLidUnit->intercepEvap = IntercepEvap;
    theLidUnit->intercepInflow = IntercepRate;
    ...
    *lidEvap = SurfaceEvap + PaveEvap + SoilEvap + StorageEvap + IntercepEvap + SoilTransEvap;  //(ue)
    *lidInfil = StorageExfil;
    *lidDrain = StorageDrain;

    //... add Rainfall to surfaceinflow to get correct water balance
    SurfaceInflow += Rainfall;                                                 //(ue)
...
//
{
    ...
    totalEvap = SurfaceEvap + PaveEvap + SoilEvap + SoilTransEvap + StorageEvap + IntercepEvap;   //(ue)
    totalVolume = SurfaceVolume + PaveVolume + SoilVolume + StorageVolume + IntercepVolume;       //(ue)
    ...
    if (lidUnit->rptFile)
    {
        ...
        //... ue reporting variables                                          //(ue)
        rptVars[VEGE_INFLOW] = theLidUnit->intercepInflow * ucf;
        rptVars[VEGE_EVAP] = IntercepEvap * ucf;
        rptVars[SOIL_EVAP] = SoilEvap * ucf;
        rptVars[SOIL_TRANSEVAP] = SoilTransEvap * ucf;
        rptVars[SURF_EVAP] = SurfaceEvap * ucf;

        //... ue potential evaporation rates                                  //(ue)
        rptVars[EP] = potEp * ucf;
        rptVars[EIP] = potEip * ucf;
        rptVars[ESP] = potEsp * ucf;
        rptVars[ETP] = potEtp * ucf;
        ...
        //... ue reporting variables                                          //(ue)
        rptVars[VEGE_DEPTH] = theLidUnit->intercepDepth * ucf;
        ...
        sprintf(theLidUnit->rptFile->results,
            ...
            "%8.3f\t %8.3f\t %8.3f\t %8.3f\t %8.3f\t %8.3f\t %8.3f",   //(ue)
            ...
            rptVars[12], rptVars[13], rptVars[14],
            rptVars[15], rptVars[16], rptVars[17],
            rptVars[EP], rptVars[EIP], rptVars[ESP], rptVars[ETP]);            //(ue)
...
////  This function was re-written for release UrbanEva ////                 //(ue)
...
//
{
    ...
    double storageThickness = theLidProc->storage.thickness;
    double storageVoidFrac = theLidProc->storage.voidFrac;
    double intercepMaxDepth = theLidUnit->intercepMaxDepth;                    //(ue)
    double intercepSCF = theLidUnit->intercepSCF;                         //(ue)
    ...
    soilTheta = x[SOIL];
    storageDepth = x[STOR];
    intercepDepth = x[VEGE];                                                   //(ue)

    //... convert moisture levels to volumes
    SurfaceVolume = surfaceDepth * theLidProc->surface.voidFrac;
    SoilVolume = soilTheta * soilThickness;
    StorageVolume = storageDepth * storageVoidFrac;
    IntercepVolume = intercepDepth * intercepSCF;                              //(ue)

    //... get ET rates
    availVolume = SoilVolume - soilWiltPoint * soilThickness;
    //getEvapRates(SurfaceVolume, 0.0, availVolume, StorageVolume, 1.0);
    getEvapRatesIntercep(SurfaceVolume, IntercepVolume,
        availVolume, StorageVolume, 1.0);                                      //(ue)

    if (soilTheta >= soilPorosity) StorageEvap = 0.0;

    // calc interception here                         
    IntercepRate = getInterceptionRate(intercepDepth);                         //(ue)

    // insert infiltration calculation here use mod grnampt                    //(ue)
    // moved from function lidproc_getOutflow option for selection, currently not used
    SurfaceInfil =
        grnampt_getInfil(&theLidUnit->soilInfil, Tstep,
            Rainfall - IntercepRate + SurfaceInflow, theLidUnit->surfaceDepth,
            MOD_GREEN_AMPT);
    ...
    //... compute overall layer flux rates
    f[SURF] = ((Rainfall - IntercepRate) + SurfaceInflow - SurfaceEvap - SurfaceInfil - SurfaceOutflow) /     //(ue)
        theLidProc->surface.voidFrac;
    f[SOIL] = (SurfaceInfil - SoilEvap - SoilTransEvap - SoilPerc) /
        theLidProc->soil.thickness;
    if (storageThickness == 0.0) f[STOR] = 0.0;
    else f[STOR] = (SoilPerc - StorageEvap - StorageExfil - StorageDrain) /
        theLidProc->storage.voidFrac;

    // ...add flux rate for interception layer
    // ...calc delta S in interception storage
    if (intercepMaxDepth <= 0) f[VEGE] = 0;                                    //(ue)
    else f[VEGE] = (IntercepRate - IntercepEvap) /
        theLidUnit->intercepSCF;
}
...
// this function was inserted for release UrbanEva
double getInterceptionRate(double depth)                                //(ue)
//
//  Purpose: computes interception rate of a LID's vegetation/interception layer.
//  Input:   depth = depth of water on vegetation/interception layer (ft)
//  Output:  returns interception rate of vegetation layer (ft/s)
//
{
    ...
    double delta = 0;
    double interceptionrate = 0;
    int interceptype = theLidProc->VEGETAT.interceptionType;

    // limit interception rate if storage is full
    delta = theLidUnit->intercepMaxDepth - depth;
    if (delta < 0) return 0.0;

    // select interceptiontype and calculate
    if (interceptype == 0)
    {
        // no interception
        interceptionrate = 0;
    }
    else if (interceptype == 3)
    {
        // calc interception rate by Braden (first in mm/h)
        double rainrate_mmh = Rainfall * UCF(RAINFALL);
        interceptionrate = theLidProc->VEGETAT.SL * theLidUnit->intercepLAI *
            (1 - 1 / (1 + theLidUnit->intercepSCF * rainrate_mmh / (theLidProc->VEGETAT.SL * theLidUnit->intercepLAI)));// currently in mm/h

        interceptionrate /= UCF(RAINFALL);									   // recalc to ft/sec

    }
    else if (interceptype == 2) {
        // constant interception rate for soil covered fraction
        interceptionrate = theLidUnit->intercepSCF * Rainfall;

    }
    else {
        // constant interception rate for total area
        interceptionrate = Rainfall;

    }

    interceptionrate = MIN(interceptionrate, delta / Tstep);
...
////  This function was inserted for release UrbanEva                          //(ue)

void getEvapRatesIntercep(double surfaceVol, double intercepVol, double soilVol,
    double storageVol, double pervFrac)    //(ue)
    //
    ...
    //           intercepVol = volume/area of water in interception storage (ft)   
    ...
    //
{
    ...
    // global variables for calc return
    IntercepEvap = 0.0;
    SurfaceEvap = 0.0;
    SoilEvap = 0.0;
    StorageEvap = 0.0;
    ...
    //... return with everything = 0 if no pot. evap
    if (EvapRate <= 0) return;

    //... use cropfactor for pot. evap
    double vegetatCropFactor = theLidProc->VEGETAT.cropfactor;
    double Estip = EvapRate * vegetatCropFactor;
    potEp = Estip;
    //... declare potential rates, 
    //... i=interception, st=combined soil evap and transpiration, s=soil, t=transpiration
    double Eip, Estp, Esp, Etp;

    // calc timestep variables for vegetation layer
    double vegetatSCF = theLidUnit->intercepSCF;
    double vegetatSmax = theLidUnit->intercepMaxDepth;
    double factor = 0;

    //... check if interception evap has to be calculated
    if (theLidProc->VEGETAT.interceptionType > 0 && intercepVol > 0 && (theLidProc->VEGETAT.evapWhileponding == 1 || surfaceVol <= 0))
    {
        factor = pow(intercepVol / vegetatSmax, 2.0 / 3.0);

        Eip = Estip;
        //... if interception model != "all Rainfall=1", consider only vegetated fraction
        if (theLidProc->VEGETAT.interceptionType > 1) Eip *= vegetatSCF;
        if (theLidProc->VEGETAT.intercepEvapType == 2) Eip *= factor;
        potEip = Eip;
        IntercepEvap = MIN(intercepVol / Tstep, Eip);
    }

    //... calc evap form ponding water 
    if (surfaceVol > 0)
    {
        SurfaceEvap = MIN(surfaceVol / Tstep, Estip);
        //... end evap if no soilevap and transp while ponding
        if (theLidProc->VEGETAT.evapWhileponding == 0) return;
    }

    //... set soil parameters

    double wp = theLidProc->soil.wiltPoint;
    double fc = theLidProc->soil.fieldCap;
    double nfc = fc - wp;
    // this seem to be a bug, adding Wiltingpoint is necessary!!!
    // old Version form 5.1.012ue: double soilTheta = soilVol / theLidProc->soil.thickness; 
    // corrected version
    double soilTheta = soilVol / theLidProc->soil.thickness + wp;

    double wrel = MIN(1, MAX(0, (soilTheta - wp) / nfc));

    //... check if evap&transpiration as combined process evaporationTyp 4 or 5
    if (theLidProc->VEGETAT.evaporationType > 3)
    {
        Estp = Estip - IntercepEvap;
        potEsp = Estp;
        if (theLidProc->VEGETAT.evaporationType == 4)
        {
            //... use method from disse 1995

            double r = theLidUnit->intercepLAI;
            Esp = (1.0 - exp(-r * wrel)) / (1.0 - 2.0 * exp(-r) + exp(-r * wrel));
            //... check between 0 and 1
            Esp = MIN(1, MAX(0, Esp)) * Estp;

        }
        else if (theLidProc->VEGETAT.evaporationType == 5)
        {
            //... use method from ostrowski 1992
            if (soilTheta < (wp + 0.2 * nfc))
            {
                Esp = 0;
            }
            else if (soilTheta <= (wp + 0.7 * nfc))
            {
                Esp = Estp * (soilTheta - (wp + 0.2 * nfc)) / ((wp + 0.7 * nfc) - (wp + 0.2 * nfc));
            }
            else
            {
                Esp = Estp;
        ...
        SoilEvap = MIN(Esp, soilVol / Tstep);
        return;
    }

    //... calc soil evaporation, if no ponding
    if (!surfaceVol > 0)
    {
        Esp = (1 - vegetatSCF) * Estip;
        potEsp = Esp;
        double se_fac = 1;                                    // evaporationType==0
        if (theLidProc->VEGETAT.evaporationType == 1)
            se_fac = wrel / theLidProc->VEGETAT.nfcThreshold;
        else if (theLidProc->VEGETAT.evaporationType == 2)
            se_fac = pow(wrel / theLidProc->VEGETAT.nfcThreshold, 0.5);
        else if (theLidProc->VEGETAT.evaporationType == 3)
            se_fac = pow(wrel / theLidProc->VEGETAT.nfcThreshold, 2.0);

        SoilEvap = MIN(MIN(1, MAX(0, se_fac)) * Esp, soilVol / Tstep);

    }

    //... calc available veget evap rate
    Etp = vegetatSCF * Estip - IntercepEvap;
    potEtp = Etp;

    if (Etp > 0)
    {
        Etp = vegetatSCF * Estip - IntercepEvap;
        double te_fac = 1;                                    // transpirationType==0
        if (theLidProc->VEGETAT.transpirationType == 1)
            te_fac = (1 - factor);
        else if (theLidProc->VEGETAT.transpirationType == 2)
            te_fac = wrel / theLidProc->VEGETAT.nfcThreshold;

        SoilTransEvap = MIN(MIN(1, MAX(0, te_fac)) * Etp, soilVol / Tstep - SoilEvap);  // priority on soil evaporation if soil water is limiting
    }
    ...
    //
{
    ...
    //... 4 waterBalance variables for evaporation                             //(ue)
    lidUnit->waterBalance.intercepEvap += IntercepEvap * Tstep;
    lidUnit->waterBalance.surfaceEvap += SurfaceEvap * Tstep;
    lidUnit->waterBalance.soilEvap += SoilEvap * Tstep;
    lidUnit->waterBalance.soilTransp += SoilTransEvap * Tstep;
}
