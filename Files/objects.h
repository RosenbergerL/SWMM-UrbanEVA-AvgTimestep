...
//
//  Build UrbanEva (ue):
//   - add evapPot and evapfactors to TSubcatch
// 
//   Build UrbanEva-AvgTimestep (ue-avg):
//   - add txtfile to TRptFlags
//-----------------------------------------------------------------------------
...
typedef struct
{
   ...
   // -----------------------------                                            //(ue)
   double        evapfactors[3];  // evaporation factor for 3 different saisons //(ue)
   double        evapPot;         // potential evap rate (ft/sec)              //(ue)
}  TSubcatch;
...
typedef struct
{
   ...
   char          txtfile;         // TRUE if txtfile export active              //(ue-avg)
}  TRptFlags;
