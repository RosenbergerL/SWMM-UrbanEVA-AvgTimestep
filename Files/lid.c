...
//
//  Build UrbanEva (ue):
//   - Layer VEGETAT added to LidLayerTypes, LidTypeWords
//   - readVEGETATData and lid_readProcParams: Reading parameters of Layer VEGETAT
//   - Reporting : initLidRptFile and lid_writeWaterBalance
//-----------------------------------------------------------------------------
...
"VEGETAT","REMOVALS", NULL };                                                 //(5.1.013)&(ue)
...
static int    readVEGETATData(int j, char* toks[], int ntoks);                 //(ue)
...
//
{
    ...
    case VEGETAT: return readVEGETATData(j, toks, ntoks);                      //(ue)
...
////  This function was added for release UrbanEva  ////                       //(ue)

int readVEGETATData(int j, char* toks[], int ntoks)                            //(ue)
//
//  Purpose: reads Vegetation Layer data for a LID process from line of input
//           data file
//  Input:   j = LID process index 
//           toks = array of string tokens
//           ntoks = number of tokens
...
//
//  Format of data is:
//    LID_ID VEGETAT laimean cropfactor SL nfc_threshold vegetationpattern soilevapmodel intercepmodel ievapmodel soiltransmodel evapWhileponding infilmodel 
//
{
    int    i, n;
    int    soilevapmodel, infilmodel, intercepmodel, ievapmodel, soiltransmodel, evapwhileponding;
    double x[4];

    //... read numerical parameters
    if (ntoks < 6) return error_setInpError(ERR_ITEMS, "");

    // ... check if parameter are from type double 
    for (i = 2; i < 6; i++)
    {
        if (!getDouble(toks[i], &x[i - 2]) || x[i - 2] < 0.0)
            return error_setInpError(ERR_NUMBER, toks[i]);
    ...
    n = project_findObject(TIMEPATTERN, toks[6]);
    if (n < 0) return error_setInpError(ERR_NAME, toks[6]);

    //... soilevapmodel 
    if (!getInt(toks[7], &soilevapmodel) == 1)
        return error_setInpError(ERR_NUMBER, toks[7]);

    //... intercepmodel 
    if (!getInt(toks[8], &intercepmodel) == 1)
        return error_setInpError(ERR_NUMBER, toks[8]);

    //... ievapmodel
    if (!getInt(toks[9], &ievapmodel) == 1)
        return error_setInpError(ERR_NUMBER, toks[9]);

    //... soiltransmodel 
    if (!getInt(toks[10], &soiltransmodel) == 1)
        return error_setInpError(ERR_NUMBER, toks[10]);

    //... evapwhileponding
    if (!getInt(toks[11], &evapwhileponding) == 1)
        return error_setInpError(ERR_NUMBER, toks[11]);

    //... infilmodel
    if (!getInt(toks[12], &infilmodel) == 1)
        return error_setInpError(ERR_NUMBER, toks[12]);

    //... save parameters to LID Vegetation layer structure
    LidProcs[j].VEGETAT.laimean = x[0];
    LidProcs[j].VEGETAT.cropfactor = x[1];
    LidProcs[j].VEGETAT.SL = x[2];
    LidProcs[j].VEGETAT.nfcThreshold = x[3];
    LidProcs[j].VEGETAT.vegetationpattern = n;
    LidProcs[j].VEGETAT.evaporationType = soilevapmodel;
    LidProcs[j].VEGETAT.interceptionType = intercepmodel;
    LidProcs[j].VEGETAT.intercepEvapType = ievapmodel;
    LidProcs[j].VEGETAT.transpirationType = soiltransmodel;
    LidProcs[j].VEGETAT.evapWhileponding = evapwhileponding;
    LidProcs[j].VEGETAT.infiltrationType = infilmodel;
...
//
{
    ...
    //use potentil evpaporation from subcatchment with reduction factor for shading
    EvapRate = Subcatch[j].evapPot;                                            //(ue)
    ...
    //
{
    ...
    lidRunoff = lidproc_getOutflow(lidUnit, lidProc, Subcatch[j].rainfall,     //(ue) add rain
        lidInflow, EvapRate,
...
////  This function was modified for UrbanEva ////                             //(ue)
...
//
{
    ...
    ////  Headings modified for release UrbanEva  ////                         //(ue)
    fprintf(Frpt.file,
        "\n  ------------------------------------------------------------------------------------------------------------------------------------------------------------"
        "\n                                         Total      Evap     Infil   Surface    Drain    Initial     Final      Evap      Evap      Evap    TrEvap  Continuity"
        "\n                                        Inflow      Loss      Loss   Outflow   Outflow   Storage   Storage  Intercep   Surface      Soil      Soil       Error");
    if (UnitSystem == US) fprintf(Frpt.file,
        "\n  Subcatchment      LID Control             in        in        in        in        in        in        in        in        in        in        in           %%");
    else fprintf(Frpt.file,
        "\n  Subcatchment      LID Control             mm        mm        mm        mm        mm        mm        mm        mm        mm        mm        mm           %%");
    fprintf(Frpt.file,
        "\n  ------------------------------------------------------------------------------------------------------------------------------------------------------------");
    ...
    for (j = 0; j < GroupCount; j++)
    {
        ...
        while (lidList)
        {
            ...
            fprintf(Frpt.file, "\n  %-16s  %-16s", Subcatch[j].ID,
                LidProcs[k].ID);
            fprintf(Frpt.file, "%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f",     //(ue)
                ...
                lidUnit->waterBalance.intercepEvap * ucf,                      //(ue)
                lidUnit->waterBalance.surfaceEvap * ucf,                       //(ue)
                lidUnit->waterBalance.soilEvap * ucf,                          //(ue)
                lidUnit->waterBalance.soilTransp * ucf	                       //(ue)
            );
...
//
{
    ...
    static char* head1[] = {
        ...
        "     Soil\t", "     Soil\t", "  Surface\t", "       Ep\t", "      Eip\t", "      Esp\t", "      Etp" };
    static char* head2[] = {
        "\n                    \t", "     Time\t",
        ...
        " Moisture\t", "    Level\t",
        "    Level\t", "   Inflow\t", "     Evap\t",
        "     Evap\t", "   Transp\t", "     Evap\t", "       Ep\t", "      Eip\t", "      Esp\t", "      Etp" };
    static char* units1[] = {
        "\nDate        Time    \t", "    Hours\t",
        ...
        "  Content\t", "   inches\t",
        "   inches\t", "    in/hr\t", "    in/hr\t",
        "    in/hr\t", "    in/hr\t", "    in/hr\t", "    in/hr\t", "    in/hr\t", "    in/hr\t", "    in/hr" };
    static char* units2[] = {
        "\nDate        Time    \t", "    Hours\t",
