...
//
//  Build UrbanEva (ue):
//   - subcatch_readParams: reading 3 different evaporation factors for
//     modifying potential evaporation
//   - function getNetEvap added: use of 3 evaporation factors to modify 
...
static double getNetEvap(int j);
...
//
{
    ...
    x[8] = -1;
    if (ntoks == 9)
        // if exactly 9 ntoks, than Snowmelt is used                           //(ue)
    {
    ...
    // to do (ue)                                                             //(ue)
    if (ntoks > 9)
        // if more than 9 toks evaporation last 3 entries are for evaporation
    {
        for (i = ntoks - 3; i < ntoks; i++)
        {
            if (!getDouble(tok[i], &x[i]) || x[i] < 0.0)
                return error_setInpError(ERR_NUMBER, tok[i]);
        }
        for (i = 0; i < 3; i++)
        {
            Subcatch[j].evapfactors[i] = x[ntoks - 3 + i];
        }
    ...
    // --- create the snow pack object if it hasn't already been created       //(ue)
    if (ntoks == 9)
    {
...
//
{
    ...
    // --- use function for calculation of evap factor                         
    else evapRate = getNetEvap(j); //;                                         //(ue)
...
double getNetEvap(int j) {
//
//  Purpose: Calculate reduced evaportaion by shadowing
//  Input:   j = subcatchment index
//  Output:  netEvap = evap for current timestep (ft/s)
//

    double evapRate = 0.0;             // potential evaporation rate (ft/sec)
    double evapFac = 1.0;              // factor for reduction of potential evaporation (-) 
    int    doy;                        // day of year for evaporation calculation           
    
    double ks_winter  = Subcatch[j].evapfactors[0];  //evap factor for winter
    double ks_spring  = Subcatch[j].evapfactors[1];  //evap factor for spring and fall
    double ks_summer  = Subcatch[j].evapfactors[2];  //evap factor for summer
    double ks_fall    = Subcatch[j].evapfactors[1];  //evap factor for spring and fall
    double ks_eoy     = 0.791;
    int    doy_spring = 79;            // day of year start spring
    int    doy_summer = 172;           // day of year start summer
    int    doy_fall   = 263;           // day of year start fall
    int    doy_winter = 355;           // day of year start winter
    int    doy_eoy    = 366;           // day end of year
    
    doy = datetime_dayOfYear(getDateTime(NewRunoffTime));
    //...calculate ks for eoy for easy interpolation
    ks_eoy = ks_winter + (ks_spring - ks_winter) / (doy_spring + (doy_eoy - doy_winter)) * (doy_eoy - doy_winter);
    
    if (doy < doy_spring) {
        //start of the year to spring
        evapFac = ks_eoy + ((ks_spring - ks_eoy) / (doy_spring)) * (doy);
    }
    else if (doy >= doy_spring & doy < doy_summer) {
        //spring to summer
        evapFac = ks_spring + ((ks_summer - ks_spring) / (doy_summer - doy_spring)) * (doy - doy_spring);
    }
    else if (doy >= doy_summer & doy < doy_fall) {
        //summer to fall
        evapFac = ks_summer + ((ks_fall - ks_summer) / (doy_fall - doy_summer)) * (doy - doy_summer);
    }
    else if (doy >= doy_fall & doy < doy_winter) {
        //fall to winter
        evapFac = ks_fall + ((ks_winter - ks_fall) / (doy_winter - doy_fall)) * (doy - doy_fall);
    }
    else if (doy >= doy_winter) {
        //winter to eoy
        evapFac = ks_winter + ((ks_eoy - ks_winter) / (doy_eoy - doy_winter)) * (doy - doy_winter);
    }
  
    evapRate = Evap.rate * evapFac;
    // save evap rate in Subcatch struc to use in lidproc
    Subcatch[j].evapPot = evapRate;
